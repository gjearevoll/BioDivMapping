...
processMareano <- function(focalEndpoint, tempFolderName, datasetName, regionGeometry, focalTaxon, crs) {
  ...
  occurencesSF <- st_as_sf(occurrence,                         
                           coords = c("decimalLongitude", "decimalLatitude"),
                           crs = "+proj=longlat +ellps=WGS84")
  sf_use_s2(FALSE)
  occurencesSF <- st_intersection(st_transform(occurencesSF, crs), regionGeometry)
  sf_use_s2(TRUE)
  
  # Get a list of all the years
  uniqueYears <- unique(occurencesSF$year)
  uniqueYears <- uniqueYears[uniqueYears > 1994]
  
  
  allSpecies <- unique(occurencesSF$scientificName)
  ...
  fullYearList <- lapply(uniqueYears, FUN = function(x) {
    ...
    occurrenceOneYear <- as.data.frame(occurencesSF) %>%
      filter(year == x)
    ...
    if (length(allSamplingEvents) == 0) {return(NULL)}
