...
if ("birds" %in% focalTaxon$taxa) {
  specData[["TOVData"]] <- st_transform(readRDS("localArchive/birdDataTOV.RDS"), st_crs(specData[[1]])) |>
...
specDataOcc <- do.call(rbind, lapply(specData, FUN = function(x) {
  x <- x[x$simpleScientificName %in% repDataSpecies,]
  if ("individualCount" %in% colnames(x)) {
    x <- x[x$individualCount > 0,]
  }
  st_drop_geometry(x[,c("simpleScientificName", "year")])
}))
...
saveRDS(finalSpeciesList$simpleScientificName, paste0(folderName, "/segmentList.RDS"))
