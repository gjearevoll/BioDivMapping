...
source(textConnection(readLines("../pipeline/import/speciesImport.R")[21:22]))
...
infoNames <- c("Date",
               ...
               "Total species",
               "Total datasets used")
info <- c(dateAccessed, paste(focalTaxa$taxa, sep = ", "), 
          paste(focalTaxa$key, sep = ", "), level, region, nrow(focalSpecies),
...
for (i in 1:length(unique(focalTaxa$taxa))) {
  focalTaxa <- unique(focalTaxa$taxa)[i]
  processedDataShortened <- processedDataCompiled[processedDataCompiled$taxa == focalTaxa,]
